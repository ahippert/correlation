...
def correlation_ZNCC(masterImage, slaveImage, corr_window, research_window, line_start, col_start, line_width, col_width, A, B) :
    ...
    #peak = np.ndarray((line_width-research_window, col_width-research_window))
    ...
    for x in range(line_start, line_start + line_width - 2*peak_size, im_step):
        print("progressing in line... %d" %x)
        for y in range(col_start, col_start + col_width - 2*peak_size, im_step):
        
            sim = 0

            # compute master correlation window average centered at (x,y)
            avgMaster = get_mean(masterImage, x + peak_size/2, y + peak_size/2, corr_window)
        
            # slide in research window
            for p in range(0, peak_size + 1, research_step):
                for q in range(0, peak_size + 1, research_step):
                    ...
                    for i in range(0, 2*corr_window + 1):
                        for j in range(0, 2*corr_window + 1):
                    ...
                    if (c2 != 0 and c3 != 0) :
                        sim_peak[p][q] = float(c1)/((c2*c3)**0.5)
                    else :
                        sim_peak[p][q] = None
            ...
            if compute_similarity == 'py' :
                peak[x-line_start+peak_size][y-col_start+peak_size] = np.max(sim_peak)
