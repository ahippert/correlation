...
def correlation_ZNCC(masterImage, slaveImage, corr_window, research_window, line_start, col_start, line_width, col_width, A, B) :
    ...
    # we use half sizes of both correlation and research windows
    corr = corr_window/2
    research = research_window/2
    
    peak_size = research_window - corr_window + 1 # size of similarity peak research map
    ...
    offset = research # research window is first centered at (x+offset, y+offset)
    
    # faugeras (1993) approach
    # correlation window has dimensions (2n+1)(2m+1) if rectangular
    # in our case n = m (squared window)
    for x in range(line_start+offset, line_start+line_width-offset, im_step):
        print("progressing in line... %d" %x)
        for y in range(col_start+offset, col_start+col_width-offset, im_step):
        
            sim = 0

            # compute master correlation window average centered at (x,y)
            avgMaster = get_mean(masterImage, x, y, corr)
        
            # slide in research window
            for p in range(-peak_size/2, peak_size/2 + 1, rstep):
                for q in range(-peak_size/2, peak_size/2 + 1, rstep):

                    # correlation terms initialization 
                    c1, c2, c3 = (0,)*3

                    # compute average of slave correlation window centered at (x+p,y+q)
                    avgSlave = get_mean(slaveImage, x+p, y+q, corr)
                
                    # compute correlation between master & slave correlation windows
                    c1 += (masterImage[:-peak_size+1, :] - avgMaster)*(slaveImage[x+p][y+q] - avgSlave)
                    c2 += (masterImage[x][y] - avgMaster)**2
                    c3 += (slaveImage[x+p][y+q] - avgSlave)**2

                    if (c2 != 0 and c3 != 0) :
                        sim_peak[p+peak_size/2][q+peak_size/2] = float(c1)/((c2*c3)**0.5)
                    else :
                        sim_peak[p+peak_size/2][q+peak_size/2] = 0.
    ...
    print("computation time: %s s" % (time.time() - start_time))
    # graphics

    plt.figure(1)
    im = plt.imshow(peak)
    plt.colorbar(im)
    plt.title("image size=(%d*%d) ; n=%d ; m=%d" %(line_width, col_width, corr_window, research_window))

    if savefig :
        plt.savefig('/home/hipperta/corr/python/simpeak_xy%d%d_n%d_m%d.png' %(line_width, col_width, corr_window, research_window))
